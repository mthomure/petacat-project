...
function [data,model] = hmaxq_fn2c1( model, fnames )

    % [data,model] = hmaxq_fn2c1( model, fnames );
...
tic;
    ...
    % load and store the image
        ...
        data(fni).im_r = x; % save off the resized input image as well.
    ...
    % split into color bands
        ...
        %   seems to improve it substantially.
            data(fni).c1max = squeeze( max( max( data(fni).c1a_r ) ) );
            
        % compute per image salience, based on feature integration theory
        % of attention
            salience_stack = data(fni).c1a_r;
            for i = size(data(fni).c1a_r,3)
                salience_stack(:,:,i) = mat2gray(salience_stack(:,:,i));
                salience_stack(:,:,i) = (1-mean(mean(salience_stack(:,:,i)))).^2 * salience_stack(:,:,i);
            end
            data(fni).salience = sum( salience_stack, 3 );
            data(fni).salience_r = ...
                imresize( ...
                    data(fni).salience, ...
                    [size(data(fni).im,1) size(data(fni).im,2)], ...
                    'nearest');
