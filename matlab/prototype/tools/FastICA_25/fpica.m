 ...
 case 'guess'
  if size(guess,1) ~= size(whiteningMatrix,2)
	...
	fprintf('Warning: initial guess only for first %d components. Using random initial guess for others.\n', size(guess,2)); 
      ...
      % EDIT max quinn, the rand part wasn't the same size as the initial
      % guess. for some reason it was using the vectorSize, which is based
      % the size of X, which has already gone through PCA (?), but the
      % inital condition is given in the original vector size. The
      % whitening matrix below is set up for a guess that's the same size
      % as the original data, so I just changed the size of the random
      % padding. Here's hoping...
      
      guess(:, size(guess, 2) + 1:numOfIC) = ...
	  				     rand(size(guess,1),numOfIC-size(guess,2))-.5;
