function [y] = retinal( x, rp )
    % [retinal_output] = retinal( x, rp )
    ...
    %
    % [retinal_output] = hm1_retinal_processing( mat2gray(imread('cameraman.tif')), 15 );
    ...
    %         subplot(1,2,2); imshow(retinal_output + .5);
        ...
        % approximate locally standard normal, maybe with noise cutoff
            y = (x - x_mu) ./ max(x_sigma, .1);
            % y = (x - x_mu) ./ (x_sigma + .1);
            % y = (x - x_mu) ./ (x_sigma + 1 - blackman_cdf( x_sigma, 0, .1)); 
